 ...
 *
 * NOTE: this test does not import FixedClockTestConfig. That test-only config freezes
 * the app's injected Clock bean, but JWT expiry is checked by the JJWT library against
 * the real system clock, not the app's Clock bean — freezing the clock far from the
 * real date makes every generated token look already-expired and every authenticated
 * call fail with 403. None of the assertions here need a deterministic date, so the
 * real clock is used.
 */
@SpringBootTest
